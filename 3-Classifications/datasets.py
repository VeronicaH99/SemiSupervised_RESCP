...
class Dataset_call(object):
    def __init__(self, dataset_name, root, patch_length=4, batch_size=16, return_zero_labels=False,
                 sample_num=10, validation_ratio=0.3):

        if dataset_name == 'UP':
            data_dir = os.path.join(root, 'UP/')
            data = sio.loadmat(data_dir + "PaviaU.mat")
            label = sio.loadmat(data_dir + 'UP_test_set_idx.mat')
            p_label = sio.loadmat(data_dir + 'new_train.mat')
            self.data = data["paviaU"]
            self.label = label["test_set_idx_UP"]
            self.p_label = p_label['new_train']
        if dataset_name == "SA":
            data_dir = os.path.join(root, 'SA/')
            data = sio.loadmat(data_dir + "Salinas_corrected.mat")
            label = sio.loadmat(data_dir + 'SA_test_set_idx.mat')
            p_label = sio.loadmat(data_dir + 'new_train.mat')
            self.data = data["salinas_corrected"]
            self.label = label["test_set_idx_SA"]
            self.p_label = p_label['new_train']
        if dataset_name == "KSC":
            data_dir = os.path.join(root, 'KSC/')
            data = sio.loadmat(data_dir + "KSC.mat")
            label = sio.loadmat(data_dir + 'KSC_test_set_idx.mat')
            p_label = sio.loadmat(data_dir + 'new_train.mat')
            self.data = data["KSC"]
            self.label = label["test_set_idx_KSC"]
            self.p_label = p_label['new_train']
        if dataset_name == "UH":
            data_dir = os.path.join(root, 'UH/')
            data = sio.loadmat(data_dir + "Houston.mat")
            label = sio.loadmat(data_dir + 'UH_test_set_idx.mat')
            p_label = sio.loadmat(data_dir + 'new_train.mat')
            self.data = data["Houston"]
            self.label = label["test_set_idx_UH"]
            self.p_label = p_label['new_train']

        self.Input_dimension = self.data.shape[2]
        self.PATCH_LENGTH = patch_length
        self.gt = self.label.reshape(np.prod(self.label.shape[:2]), )
        self.p_gt = self.p_label.reshape(np.prod(self.p_label.shape[:2]),)
        self.batch_size = batch_size
        self.padded_data = self.pre_processing()
        self.sample_num = sample_num
        self.train_indices, self.val_indices = self.sampling(validation_ratio)
        self.test_indices = self.test_idx_all()
    ...
    def sampling(self, proportion):
        # proportion = self.sample_num
        ground_truth = self.p_gt
        train = {}
        test = {}
        labels_loc = {}
        m = max(ground_truth)
    ...
    def generate_iter(self):
        ...
        val_data = self.select_small_cubic(len(self.val_indices), self.val_indices, self.data,
                                        self.PATCH_LENGTH, self.padded_data, self.Input_dimension)
        ...
        x_val = val_data.reshape(val_data.shape[0], val_data.shape[1], val_data.shape[2], self.Input_dimension)
        x_test_all = test_data.reshape(test_data.shape[0], test_data.shape[1], test_data.shape[2], self.Input_dimension)

        #
        x_test = x_test_all
        y_train = self.p_gt[self.train_indices] - 1
        y_val = self.p_gt[self.val_indices] - 1
        y_test = self.gt[self.test_indices] - 1
        # print(len(y_test))
        ...
        #
        x1_tensor_val = torch.from_numpy(x_val).type(torch.FloatTensor).unsqueeze(1)
        y1_tensor_val = torch.from_numpy(y_val).type(torch.FloatTensor)
        torch_dataset_val = TensorDataset(x1_tensor_val, y1_tensor_val)
        #
        ...
        if self.return_zero_labels == True:
            ...
            return train_iter, val_iter, test_iter, all_iter  # , valiada_iter # , y_test
    ...
    def get_iterator(self):
        ...
        #
        if self.return_zero_labels == True:
            train_iter, val_iter, test_iter, all_iter = self.generate_iter()  # batchsize in 1
            return train_iter, val_iter, test_iter, all_iter
        else:
            train_iter, val_iter, test_iter = self.generate_iter()
            return train_iter, val_iter, test_iter

# dataset = Dataset_call(root="../Datasets/", patch_length=4, dataset_name="UP",
#                                         return_zero_labels=False)
# trainloader, valloader, testloader = dataset.get_iterator()
